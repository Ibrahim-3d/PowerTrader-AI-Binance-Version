...
class PowerTraderHub(tk.Tk):
    ...
    def open_settings_dialog(self) -> None:
        ...
        # --- Binance API setup (writes b_key.txt + b_secret.txt used by pt_trader.py) ---
        ...
        def _open_api_folder() -> None:
            """Open the folder where b_key.txt / b_secret.txt live."""
            try:
        ...
        def _clear_api_files() -> None:
            """Delete b_key.txt / b_secret.txt (with a big confirmation)."""
            key_path, secret_path = _api_paths()
        ...
        def _open_binance_api_wizard() -> None:
            """
            Beginner-friendly wizard that creates + stores Binance API credentials.

            What we store:
              - b_key.txt    = your Binance API Key
              - b_secret.txt = your Binance Secret Key (treat like a password)
            """
            import webbrowser
            ...
            existing_api_key, existing_secret_key = _read_api_files()
            ...
            ttk.Button(top_btns, text="Open Binance API Management", command=lambda: webbrowser.open("https://www.binance.com/en/my/settings/api-management")).pack(side="left")
            ttk.Button(top_btns, text="Binance API Docs", command=lambda: webbrowser.open("https://www.binance.com/en/binance-api")).pack(side="left", padx=8)

            # -----------------------------
            # Step 1 — API Key + Secret Key
            # -----------------------------
            ...
            api_ent = ttk.Entry(step1, textvariable=api_key_var)
            ...
            secret_ent = ttk.Entry(step1, textvariable=secret_key_var, show="*")
            secret_ent.grid(row=1, column=1, sticky="ew", padx=10, pady=(4, 10))
            ...
            ack = ttk.Checkbutton(
                step2,
                text="I understand b_secret.txt is PRIVATE and I will not share it.",
                variable=ack_var,
            )
            ...
            def do_save():
                ...
                if not bool(ack_var.get()):
                    messagebox.showwarning(
                        "Please confirm",
                        "For safety, please check the box confirming you understand b_secret.txt is private."
                    )
                    return

                # Small sanity warning
                if len(api_key) < 10:
                    if not messagebox.askyesno(
                        "API key looks short",
                        "That API key looks unusually short. Are you sure you copied the right value?"
                    ):
                        return
