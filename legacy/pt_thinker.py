...
# -----------------------------
# Binance market-data (current ASK via order book ticker)
# -----------------------------
_BINANCE_CLIENT = None  # lazy-init so import doesn't explode if creds missing
...
def binance_current_ask(symbol: str) -> float:
    """
    Returns Binance current ASK price for symbols like 'BTCUSDT'.
    Reads creds from b_key.txt and b_secret.txt in the same folder as this script.
    """
    ...
    if _BINANCE_CLIENT is None:
        ...
        if not os.path.isfile(key_path) or not os.path.isfile(secret_path):
            raise RuntimeError(
                "Missing b_key.txt and/or b_secret.txt next to pt_thinker.py. "
    ...
    ticker = _BINANCE_CLIENT.get_orderbook_ticker(symbol=symbol)
...
def step_coin(sym: str):
	...
	if tf_choice_index >= len(tf_choices):
		...
		# get current price ONCE per coin — use Binance's current ASK
		bn_symbol = _to_binance_symbol(sym)
		while True:
			try:
				current = binance_current_ask(bn_symbol)
				break
			except Exception as e:
				print(e)
				continue
