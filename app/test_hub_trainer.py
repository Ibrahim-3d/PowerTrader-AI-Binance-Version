...
def _monitor_trainer_process(coin: str, proc: subprocess.Popen, check_count: int = 0) -> None:
    """Monitor trainer process like the hub does"""
    try:
        if proc and proc.poll() is not None:
            print(f"DEBUG: Trainer process for {coin} exited with code: {proc.returncode}")
            try:
            ...
            print(f"DEBUG: Trainer {coin} still running (PID: {proc.pid if proc else 'None'})")
            time.sleep(2)
...
def test_hub_trainer_launch(coin: str = "XRP") -> int:
    ...
    try:
        ...
        if info.proc.poll() is not None:
            print(f"DEBUG: Process {info.proc.pid} already terminated with exit code: {info.proc.returncode}")
            try:
