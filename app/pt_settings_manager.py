...
class SettingsValidator:
    ...
    def _setup_default_rules(self):
        ...
        self.add_rule(
            "coins",
            lambda v: isinstance(v, list) and len(v) > 0 and all(isinstance(coin, str) for coin in v),
            "Coins must be a non-empty list of strings",
            lambda v: ["BTC"] if not isinstance(v, list) or len(v) == 0 else [str(coin).upper().strip() for coin in v]
        )
        ...
        self.add_rule(
            ...
            lambda v: max(1024, min(65535, int(v) if isinstance(v, (int, float)) else 8080))
        )
        ...
        self.add_rule(
            ...
            lambda v: max(0.01, min(1.0, float(v) if isinstance(v, (int, float)) else 0.1))
        )
        ...
        self.add_rule(
            ...
            lambda v: max(0.001, min(1.0, float(v) if isinstance(v, (int, float)) else 0.05))
        )
    ...
    def add_rule(self, field_path: str, validator: Callable[[Any], bool], 
                 error_message: str, auto_fix: Optional[Callable[[Any], Any]] = None):
        """Add a validation rule."""
        rule = SettingsValidationRule(field_path, validator, error_message, auto_fix)
        self.rules.append(rule)
    
    def validate(self, settings: Dict[str, Any], auto_fix: bool = False) -> tuple[bool, List[str]]:
        ...
        for rule in self.rules:
            ...
            if value is None:
                ...
                if auto_fix and rule.auto_fix:
                    default_value = self._get_nested_value(DEFAULT_SETTINGS, rule.field_path)
...
class SettingsManager:
    ...
    def __init__(self, settings_file: str = SETTINGS_FILE, settings_dir: Optional[str] = None):
    ...
    def load_settings(self) -> bool:
        ...
        with self._lock:
            try:
                ...
                # Merge with defaults
                self._settings = self._merge_settings(DEFAULT_SETTINGS.copy(), loaded_settings)
                
                # Normalize coins
                if "coins" in self._settings:
                    self._settings["coins"] = [c.upper().strip() for c in self._settings["coins"] if str(c).strip()]
    ...
    def set(self, key: str, value: Any) -> bool:
        ...
        with self._lock:
            try:
                ...
                # Validate the change
                is_valid, errors = self.validator.validate(self._settings, auto_fix=False)
    ...
    def update(self, new_settings: Dict[str, Any], merge: bool = True) -> bool:
        ...
        with self._lock:
            if merge:
                ...
                original_settings = self._settings.copy()
                self._settings = self._merge_settings(DEFAULT_SETTINGS.copy(), new_settings)
    ...
    def get_validation_errors(self) -> List[str]:
        ...
        is_valid, errors = self.validator.validate(self._settings.copy(), auto_fix=False)
        return errors
...
def setup_settings_manager(settings_file: str = SETTINGS_FILE, 
                         settings_dir: Optional[str] = None) -> SettingsManager:
