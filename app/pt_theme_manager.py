...
class ThemeConfig:
    ...
    def __init__(self):
        ...
        self.widget_configs = {
            "button": {"padding": (3, 2), "focusthickness": 1},
            "notebook_tab": {"padding": (10, 6)},
            "chart_tab": {"padding": (10, 6)},
        }
        ...
        for name in ("TFrame", "TLabel", "TCheckbutton", "TRadiobutton"):
            try:
                self.style.configure(
                    name, background=colors["bg"], foreground=colors["fg"]
                )
            except Exception:
                pass
...
def setup_dark_theme(
    root: tk.Tk, theme_config: Optional[ThemeConfig] = None
) -> ThemeManager:
