...
class Phase3LiveDemo:
    ...
    def demonstrate_pattern_recognition(self):
        ...
        print(f"   💰 Price Range: ${sample_data[:, 3].min():.2f} - ${sample_data[:, 3].max():.2f}")
        print()
    ...
    def show_phase3_capabilities(self):
        ...
        for i, tf in enumerate(timeframes, 1):
            print(f"   {i}. {tf.replace('hour', 'h').replace('day', 'd').replace('week', 'w')}")
    ...
    def run_complete_demo(self):
        ...
        successful_analyses = sum(1 for r in analysis_results.values() if r.get('status') == 'success')
        total_analyses = len(analysis_results)
        
        print(f"📊 Analyses Completed: {successful_analyses}/{total_analyses}")
        
        if successful_analyses > 0:
            avg_time = sum(r.get('analysis_time', 0) for r in analysis_results.values() 
                          if r.get('status') == 'success') / successful_analyses
