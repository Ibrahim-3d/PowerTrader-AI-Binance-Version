...
class LogProc:
    ...
    def __init__(self, proc_info: ProcInfo, log_queue: queue.Queue, 
    ...
    def start(self) -> bool:
        ...
        with self._lock:
            ...
            try:
                ...
                self._log_message(f"Started {self.proc_info.name} (PID: {self.process.pid})")
                return True
    ...
    def _stream_output(self, stream, stream_name: str):
        ...
        try:
            ...
            self._log_message(f"Error streaming {stream_name} for {self.proc_info.name}: {e}")
        finally:
            try:
                stream.close()
            except Exception:
...
class ProcessManager:
    ...
    def register_callback(self, process_id: str, callback: Callable[[str, LogProc], None]):
...
"""
    ...
    try:
        ...
        while time.time() - start_time < 15:
            ...
            if stats:
                print(f"Process stats: PID={stats.pid}, CPU={stats.cpu_percent}%, "
