...
class DependencySecurityChecker:
    ...
    def check_vulnerable_packages(
        self, requirements: List[Tuple[str, Optional[str]]]
    ) -> List[str]:
        ...
        for package, version in requirements:
            if package.lower() in self.VULNERABLE_PACKAGES:
                issues.append(
                    f"VULNERABLE: {package} - Known security issues, consider alternatives"
                )

        return issues

    def check_version_security(
        self, requirements: List[Tuple[str, Optional[str]]]
    ) -> List[str]:
        ...
        for package, version in requirements:
            if package.lower() in self.SECURE_VERSIONS:
                ...
                if version is None:
                    issues.append(
                        f"UNPINNED: {package} - No version specified, should be >= {required_version}"
                    )
                else:
                    try:
                        if parse_version(version) < parse_version(required_version):
                            issues.append(
                                f"OUTDATED: {package} {version} - Should be >= {required_version} for security"
                            )
                    except Exception:
                        issues.append(
                            f"INVALID: {package} {version} - Cannot parse version"
                        )
    ...
    def check_missing_packages(
        ...
        for required_pkg in self.REQUIRED_PACKAGES:
            if required_pkg.lower() not in present_packages:
                issues.append(
                    f"MISSING: {required_pkg} - Required for PowerTrader functionality"
                )
    ...
    def run_security_audit(self) -> Dict[str, List[str]]:
        ...
        audit_results["vulnerable_packages"] = self.check_vulnerable_packages(
            requirements
        )
        ...
        if audit_results["vulnerable_packages"]:
            audit_results["recommendations"].append(
                "Remove or replace vulnerable packages"
            )
        ...
        if not any(
            audit_results[key]
            for key in ["vulnerable_packages", "version_issues", "missing_packages"]
        ):
    ...
    def update_requirements_file(self) -> bool:
        ...
        try:
            ...
            if os.path.exists(self.requirements_path):
                ...
                with open(self.requirements_path, "r") as src, open(
                    backup_path, "w"
                ) as dst:
                    dst.write(src.read())
        ...
        for pkg in checker.REQUIRED_PACKAGES:
            ...
            if pkg_lower in installed:
                ...
                status = (
                    ...
                    or parse_version(version) >= parse_version(required_version)
                    else "⚠️"
                )
