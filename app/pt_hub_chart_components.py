...
class CandleFetcher:
    ...
    def get_candle_data(self, symbol: str, timeframe: str = "1h", limit: int = 100) -> List[Dict]:
        """
        ...
        if (cache_key in self.cache and 
            cache_key in self.last_fetch_times and
            current_time - self.last_fetch_times[cache_key] < self.cache_timeout):
    ...
    def _generate_mock_data(self, symbol: str, limit: int) -> List[Dict]:
        ...
        candles = []
        base_price = 50000.0 if "BTC" in symbol.upper() else 3000.0 if "ETH" in symbol.upper() else 1.0
...
class CandleChart(ttk.Frame):
    ...
    def _plot_candlesticks(self):
        ...
        # Plot candlesticks
        for i, (open_price, high, low, close, date) in enumerate(zip(opens, highs, lows, closes, dates)):
        ...
        if len(closes) >= 20:
            ...
            self.ax.plot(range(19, len(closes)), sma_20, label="SMA 20", color="blue", alpha=0.7)
    ...
    def refresh_chart(self):
        ...
        # Fetch new data
        self.candle_data = self.fetcher.get_candle_data(self.symbol, self.timeframe, self.limit)
...
class AccountValueChart(ttk.Frame):
    ...
    def _setup_ui(self):
        ...
        metrics_frame = ttk.LabelFrame(control_frame, text="Performance Metrics", padding=5)
    ...
    def load_account_data(self):
        ...
        try:
            ...
            hub_data_dir = os.environ.get("POWERTRADER_HUB_DIR", 
            ...
            if os.path.exists(account_file):
                with open(account_file, 'r') as f:
                    lines = f.readlines()
                    self.account_history = [json.loads(line.strip()) for line in lines if line.strip()]
    ...
    def _generate_mock_account_data(self):
        ...
        for i in range(30):  # Last 30 days
            ...
            self.account_history.append({
                ...
                'total_value': max(value, base_value * 0.5),  # Don't go below 50% of base
                'cash': value * 0.3,
                'positions': value * 0.7
            })
    ...
    def _update_performance_metrics(self, values: List[float], peak: np.ndarray, drawdown: np.ndarray):
        ...
        self.max_drawdown_label.configure(
            ...
            foreground="red" if max_drawdown < -5 else "orange" if max_drawdown < -2 else "black"
        )
        
        self.sharpe_ratio_label.configure(
            text=f"Sharpe Ratio: {sharpe_ratio:.2f}",
            foreground="green" if sharpe_ratio > 1 else "orange" if sharpe_ratio > 0 else "red"
        )
...
class TechnicalIndicatorChart(ttk.Frame):
    ...
    def _setup_ui(self):
        ...
        ttk.Checkbutton(control_frame, text="MACD", variable=self.macd_var,
                       command=self.update_chart).pack(side=tk.LEFT, padx=5)
        ttk.Checkbutton(control_frame, text="Bollinger Bands", variable=self.bollinger_var,
                       command=self.update_chart).pack(side=tk.LEFT, padx=5)
    ...
    def _calculate_macd(self, closes: List[float]) -> Dict[str, List[float]]:
        ...
        if len(closes) >= 26:
            ...
            # Histogram
            histogram = [macd_line[len(macd_line) - len(signal_line) + i] - signal_line[i] 
    ...
    def _calculate_bollinger_bands(self, closes: List[float], period: int = 20, std_dev: float = 2) -> Dict[str, List[float]]:
    ...
    def _plot_macd(self):
        ...
        if 'macd' in macd_data and 'signal' in macd_data:
            ...
            x_signal = range(len(macd_data['macd']) - len(macd_data['signal']), len(macd_data['macd']))
            
            ax.plot(x_macd, macd_data['macd'], color='blue', linewidth=2, label='MACD')
            ax.plot(x_signal, macd_data['signal'], color='red', linewidth=2, label='Signal')
            
            if 'histogram' in macd_data:
                x_hist = range(len(macd_data['macd']) - len(macd_data['histogram']), len(macd_data['macd']))
                colors = ['green' if h > 0 else 'red' for h in macd_data['histogram']]
                ax.bar(x_hist, macd_data['histogram'], color=colors, alpha=0.7, width=0.8)
    ...
    def add_signal(self, signal_type: str, x: int, y: float, direction: str):
        ...
        if 'price' in self.axes:
            ...
            ax.scatter(x, y, color=color, marker=marker, s=100, alpha=0.8, 
                      label=f"{signal_type.upper()} Signal" if signal_type not in self.signals else "")
