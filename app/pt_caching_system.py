...
class TTLEvictionPolicy(EvictionPolicy):
    ...
    def should_evict(self, entries: Dict[str, CacheEntry], max_size: int) -> List[str]:
        ...
        if len(entries) - len(expired_keys) > max_size:
            ...
            lru_evictions = LRUEvictionPolicy().should_evict(remaining_entries, 
                                                           max_size - len(expired_keys))
...
class MemoryCache:
    ...
    def _enforce_limits(self):
        ...
        if len(self._data) > self.max_size or current_memory > self.max_memory_bytes:
            ...
            effective_max_size = min(self.max_size, 
                                   len(self._data) * self.max_memory_bytes // max(current_memory, 1))
            
            keys_to_evict = self.eviction_policy.should_evict(self._data, effective_max_size)
...
class CacheManager:
    ...
    def __init__(self, cache_dir: str = None):
        ...
        # Specialized caches
        self.market_data_cache = MemoryCache(max_size=500, max_memory_mb=50, default_ttl_seconds=60)
        self.model_cache = PersistentCache(os.path.join(self.cache_dir, "models"))
        self.config_cache = MemoryCache(max_size=100, max_memory_mb=10, default_ttl_seconds=300)
    ...
    def cache_model(self, model_id: str, model_data: Any, ttl_seconds: Optional[int] = None) -> bool:
    ...
    def cache_config(self, config_key: str, config_data: Any, ttl_seconds: int = 300) -> bool:
...
def cached(cache_manager: CacheManager, ttl_seconds: Optional[int] = None, 
          tags: List[str] = None, cache_type: str = 'memory'):
    """
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            cache = cache_manager.persistent_cache if cache_type == 'persistent' else cache_manager.memory_cache
...
if __name__ == "__main__":
    ...
    cache_manager.cache_market_data("BTCUSDT", {"price": 50000, "volume": 1000}, ttl_seconds=30)
